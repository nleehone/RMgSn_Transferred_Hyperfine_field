# Configuration file for DyMgSn at T=4 K with square wave modulated magnetic moments aligned in the basal plane
# The k-vector is (0.822, 0, 0) (incommensurate) and the magnetic structure shows first,
# third harmonics, and fifth harmonics
...
c = 15.796  # c lattice constant (Angstrom)
z_R = 0.333  # Position of the rare earth atom in cell units
z_Sn = 0.135  # Position of the Sn atom in cell units

num_cells_a = 1000  # Number of unit cells to create along a
num_cells_b = 1000  # Number of unit cells to create along b
num_cells_c = 1  # Number of unit cells to create along c

# Description of the magnetic structure
kvector = (0.830, 0.014, 0)
theta_c = 0 * Degree
theta = 90 * Degree
phi = 137 * Degree
...
def F(index, kvector, phi):
    return sign(cos(-2*pi*(np.dot(kvector, index) + phi)))
