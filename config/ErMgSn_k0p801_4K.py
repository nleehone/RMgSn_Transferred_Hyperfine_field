# Configuration file for ErMgSn at T=4 K with magnetic moments aligned along the c-axis
# The k-vector is (0.801, 0, 0) (incommensurate) and the magnetic structure shows first and third harmonics
...
a = 4.329  # a lattice constant (Angstrom)
b = 4.329  # b lattice constant (Angstrom)
c = 15.690  # c lattice constant (Angstrom)
z_R = 0.332  # Position of the rare earth atom in cell units
z_Sn = 0.137  # Position of the Sn atom in cell units
...
def F(index, kvector, phi):
    return cos(-2*pi*(np.dot(kvector, index) + phi)) - 1/3*cos(-2*pi*(np.dot(kvector, index)*3 + phi))
